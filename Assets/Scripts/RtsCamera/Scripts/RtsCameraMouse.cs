...
public class RtsCameraMouse : MonoBehaviour
{
    ...
    protected void Update()
    {
        ...
        if (AllowZoom)
        {
            ...
            if(scroll != 0)
            {
                Vector3 Dir = Camera.main.ScreenToViewportPoint(Input.mousePosition) - new Vector3(.5f, .5f, 0);
                Dir = new Vector3(Dir.x, 0, Dir.y);
                _rtsCamera.LookAt += scroll * Dir * 50;
            }

            
            //RaycastHit HitInfo;
            //if (Physics.Raycast(r, out HitInfo, 1000f, lm))
            //{
            //    //_rtsCamera.LookAt = HitInfo.point;
